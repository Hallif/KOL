...
unit ActiveKOL;{$IFDEF FPC}{$mode delphi}{$H+}{$ENDIF}
...
uses windows
	...
	,KOL
	,ActiveX
	,{$IFDEF FPC}ComObj{$ELSE}KOLComObj{$ENDIF}
	,err
	;
...
type
  ...
  public
    constructor Create(aControl: POleCtl);
  end;
  ...
  TEnumValue = record
    Value: Longint;
    Ident: {$IFDEF FPC}WideString{$ELSE}string{$ENDIF};
  end;
...
,	ISimpleFrameSite
	...
	protected
    { IUnknown }
    function QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF}IID: TGUID; out Obj): HResult; stdcall; //override;
    function _AddRef: Integer; stdcall;
    function _Release: Integer; stdcall;
    { IOleClientSite }
    ...
    function GetExtendedControl(out disp: IDispatch): HResult; stdcall;
		function TransformCoords{$IFDEF FPC}
	(	var pPtlHimetric:_POINTL;
		var pPtfContainer:tagPOINTF;
		dwFlags:LongWord
	):{$ELSE}
	(	var ptlHimetric: TPoint;
		var ptfContainer: TPointF;
			flags: Longint
	):{$ENDIF}HResult; stdcall;
		function IOleControlSite.TranslateAccelerator = OleControlSite_TranslateAccelerator;
		function OleControlSite_TranslateAccelerator{$IFDEF FPC}
		(	var pMsg:tagMSG;
			grfModifiers:LongWord
		):{$ELSE}
		(	msg: PMsg;
			grfModifiers:Longint
		):{$ENDIF}HResult; stdcall;
    ...
    function OnUIActivate: HResult; stdcall;
		function GetWindowContext{$IFDEF FPC}
	(	out ppframe:IOleInPlaceFrame;
	...
	(	out frame: IOleInPlaceFrame;
		...
		out rcPosRect: TRect;
		out rcClipRect: TRect;
		out frameInfo: TOleInPlaceFrameInfo
	):{$ENDIF}HResult; stdcall;
		function Scroll{$IFDEF FPC}
		(	scrollExtant:TSIZE
		):{$ELSE}
		(	scrollExtent:TPoint
		):{$ENDIF}HResult; stdcall;
    ...
    function OnPosRectChange{$IFDEF FPC}
    (	lprcPosRect:LPRect
    ):{$ELSE}
    (	const rcPosRect:TRect
    ):{$ENDIF}HResult; stdcall;
    { IOleInPlaceUIWindow }
    ...
    function RequestBorderSpace(const borderwidths: TRect): HResult; stdcall;
		function SetBorderSpace{$IFDEF FPC}
		(	const borderwidths:TRect
		):{$ELSE}
		(	pborderwidths:PRect
		):{$ENDIF}HResult; stdcall;
    ...
    function EnableModeless(fEnable: BOOL): HResult; stdcall;
		...
		function OleInPlaceFrame_TranslateAccelerator(var msg: Windows.TMsg; wID: Word): HResult; stdcall;
    { IDispatch }
    function GetTypeInfoCount(out Count: Integer): HResult; stdcall;
    function GetTypeInfo(Index, LocaleID: Integer; out TypeInfo): HResult; stdcall;
    function GetIDsOfNames(const IID: TGUID; Names: Pointer; NameCount, LocaleID: Integer; DispIDs: Pointer): HResult; stdcall;
    function Invoke(dispId: Integer; const IID: TGUID; LocaleID: Integer; Flags: Word; var Params; VarResult, ExcepInfo, ArgErr: Pointer): HResult; stdcall;
    { ISimpleFrameSite }
		function PreMessageFilter{$IFDEF FPC}
		(	hwnd:wireHWND;
			msg:UInt;
			wp:UINT_PTR;
			lp:LONG_PTR;
		out	plResult:LONG_PTR;
		out	pdwCookie:LongWord
		):{$ELSE}
		(	wnd: HWnd;
			msg,wp,lp:Integer;
		out	res: Integer;
		...
		function PostMessageFilter{$IFDEF FPC}
		(	hwnd:wireHWND;
			msg:UInt;
			wp:UINT_PTR;
			lp:LONG_PTR;
  ...
  protected
    ...
    FMiscStatus:{$IFDEF FPC}LongWord{$ELSE}Longint{$ENDIF};
    FFonts: PList;
    FPictures: PList;
    ...
    FPropConnection:{$IFDEF FPC}LongWord{$ELSE}Longint{$ENDIF};
    FEventsConnection:{$IFDEF FPC}LongWord{$ELSE}Longint{$ENDIF};
  ...
  public
    ...
    procedure MouseDown(Button: TMouseButton; AShift: DWORD; X, Y: Integer);
    procedure MouseMove(AShift: DWORD; X, Y: Integer);
    procedure MouseUp(Button: TMouseButton; AShift: DWORD; X, Y: Integer);
...
type
  ...
  public
    constructor Create(pServer: TOleServer);
  end;
...
constructor TEnumPropDesc.Create(dispId, ValueCount: Integer; const TypeInfo: ITypeInfo);
var
  I: Integer;
  VarDesc: PVarDesc;
  XName: WideString;
...
begin
  ...
  begin
    ...
    for I := 0 to FValueCount - 1 do
      with FValues^[I] do
        if AnsiCompareText(S, ansiString(Ident)) = 0 then
        begin
          Result := Value;
          Exit;
        end;
  raise {EOleError}err.exception.CreateResFmt(e_Ole, Integer( @SBadPropValue ), [S]);
end;
...
begin
  if FOleControl = nil then
    try
      ...
      InterfaceConnect(FOleObject, IPropertyNotifySink,  fOleCtlIntf, FPropConnection);
...
procedure TOleCtl.CreateEnumPropDescs;
  ...
  procedure CreateEnum(aTypeDesc:{$IFDEF FPC}TypeDesc{$ELSE}TTypeDesc{$ENDIF}; const TypeInfo: ITypeInfo; dispId: Integer);
  var
    RefInfo: ITypeInfo;
    RefAttr: PTypeAttr;
    epd: PEnumPropDesc;
  begin
    if aTypeDesc.vt <> VT_USERDEFINED then Exit;
    OleCheck(TypeInfo.GetRefTypeInfo(aTypeDesc.hreftype, RefInfo));
            ...
            case FuncDesc^.invkind of
              ...
              INVOKE_PROPERTYPUTREF:
                if FuncDesc^.lprgelemdescParam[FuncDesc^.cParams - 1].tdesc.vt = VT_PTR then
                  CreateEnum(FuncDesc^.lprgelemdescParam[FuncDesc^.cParams - 1].tdesc.{$IFDEF FPC}lptdesc{$ELSE}ptdesc{$ENDIF}^,
                    TypeInfo, FuncDesc^.memid);
            end;
        finally
...
var
  ...
  { IClassFactory2 status codes }
    CLASS_E_NOTLICENSED = $80040110 { CLASSFACTORY_E_FIRST } + 2;
  begin
    if Status = CLASS_E_NOTLICENSED then
      raise {EOleError}err.exception.CreateFmt(e_Ole, Ident, [SubClassName]);
    OleCheck(Status);
  end;
...
procedure CallEventMethod(const EventInfo: TEventInfo); assembler;
asm
        PUSH    EBX
        PUSH    ESI
        PUSH    EBP
...
begin
  ...
  try
    ...
    begin
      FOleObject.DoVerb(OLEIVERB_INPLACEACTIVATE, nil, fOleCtlIntf, 0, ParentWindow, BoundsRect);
      if FOleInPlaceObject = nil then
        raise {EOleError}err.exception.CreateResFmt(e_Ole, Integer( @SCannotActivate ), [nil]);
      HookControlWndProc;
...
begin
  ...
  if Integer(EventInfo.Method.Code) >= $10000 then//Hmm...
  begin
    StrCount := 0;
    try
...
begin
	...
	FPersistStream := nil;
	if FOleObject <> nil
	then FOleObject.SetClientSite(nil)
	;
	FOleObject := nil;
	FEventDispatch.Free;
	FreeList(FFonts);
	...
	if FControlData^.InstanceCount = 0
	then DestroyEnumPropDescs
	;
	fOleCtlIntf.Free;
	inherited Destroy;
end;
...
begin
  ...
  begin
    ...
    if WndHandle = 0 then
      raise {EOleError}err.exception.CreateResFmt(e_Ole, Integer(@SNoWindowHandle), [nil]);
    fHandle := WndHandle;
...
begin
  ...
  begin
    ...
    if Integer(EventMethod.Code) < $10000 then Exit;//Hmm......
...
procedure TOleCtl.MouseUp(Button: TMouseButton; AShift: DWORD; X, Y: Integer);
begin
  //TODO: mouse
end;
...
begin
	...
	then begin
		...
		if FControlData^.Flags and (cfCaption or cfText) <> 0
		then begin
			if FControlData^.Flags and cfCaption <> 0
			then dispId:=DISPID_CAPTION
			else dispId:=DISPID_TEXT
			;
			if OldName = GetStringProp(dispId)
			then SetStringProp(dispId, Value);
		end;
	end;
end;
...
begin
	...
	if Value.VType <> varDispatch
	then InvKind:=DISPATCH_PROPERTYPUT
	else InvKind:=DISPATCH_PROPERTYPUTREF
	;
	Status:=FControlDispatch.Invoke(Index, GUID_NULL, 0,InvKind, DispParams, nil, @ ExcepInfo, nil);
	if Status <> 0
	then DispatchInvokeError(Status, ExcepInfo)
end;
...
procedure TOleCtl.SetWideStringProp(Index: Integer; const Value: WideString);
var
  Temp: TVarData;
begin
  Temp.VType := varOleStr;
		...
		case dispId of
			...
			DISPID_KEYUP	:begin
				...
				then begin
					...
					if ((Args^[1].vType and varByRef) <> 0)
					then Word(Args^[1].VPointer^):=Key;
				end;
			end;
			DISPID_KEYPRESS:
...
constructor TServerEventDispatch.Create(pServer: TOleServer);
begin
	FServer:=pServer;
	InternalRefCount:=1;
end;
...
function TServerEventDispatch.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF}IID: TGUID; out Obj): HResult;
begin
...
function TServerEventDispatch.GetIDsOfNames(const IID: TGUID; Names: Pointer; NameCount, LocaleID: Integer; DispIDs: Pointer): HResult;
begin
  Result := E_NOTIMPL;
end;

function TServerEventDispatch.Invoke(dispId: Integer; const IID: TGUID; LocaleID: Integer; Flags: Word; var Params; VarResult, ExcepInfo, ArgErr: Pointer): HResult;
var
  ...
  case ConnectKind of
    ...
    begin
      ...
      begin
        ...
        raise {EOleSysError.Create}err.exception.create( e_Ole, ErrorStr {, HR, 0} );
      end;
    end;
    ...
    ckRemote:
      ...
      Result := CreateRemoteComObject({$IFDEF FPC}WideString{$ENDIF}(RemoteMachineName), FServerData^.ClassID);
  end;
end;

procedure TOleServer.ConnectEvents(const Obj: IUnknown);
begin
	{$IFnDEF FPC}KOLComObj
.	{$ENDIF}InterfaceConnect(Obj, FServerData^.EventIID, FEventDispatch, FEventsConnection);
end;

procedure TOleServer.DisconnectEvents(const Obj: Iunknown);
begin
	{$IFnDEF FPC}KOLComObj
.	{$ENDIF}InterfaceDisconnect(Obj, FServerData^.EventIID, FEventsConnection);
end;
...
{ TOleServer.IUnknown }
function TOleServer.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF}IID: TGUID; out Obj): HResult;
begin
  if GetInterface(IID, Obj) then
    Result := S_OK
  else
...
constructor TEventDispatch.Create(aControl: POleCtl);
begin
	FControl:=aControl;
end;
...
function TEventDispatch.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF}IID: TGUID; out Obj): HResult;
begin
  if GetInterface(IID, Obj) then
  begin
    Result := S_OK;
...
function TOleCtlIntf.GetIDsOfNames(const IID: TGUID; Names: Pointer; NameCount, LocaleID: Integer; DispIDs: Pointer): HResult;
begin
  Result := E_NOTIMPL;
end;

function TOleCtlIntf.GetMoniker(dwAssign, dwWhichMoniker: Integer; out mk: IMoniker): HResult;
begin
  Result := E_NOTIMPL;
end;

function TOleCtlIntf.GetTypeInfo(Index, LocaleID: Integer; out TypeInfo): HResult;
begin
  Pointer(TypeInfo) := nil;
  Result := E_NOTIMPL;
end;
...
function TOleCtlIntf.GetWindowContext({$IFDEF FPC}
out		ppframe:IOleInPlaceFrame;
out		ppdoc:IOleInPlaceUIWindow;
		lprcposrect:LPRECT;
		lprccliprect:LPRECT;
...
begin
	ppframe:=Self;
	ppdoc:=nil;
	lprcposrect^:=fOleCtl.BoundsRect;
	lprccliprect^:=MakeRect(0, 0, 32767, 32767);
	with lpframeinfo^
	do begin
		fMDIApp:=false;
	...
	GetWindowContext:=S_OK;
end;{$ELSE !FPC}
out		frame: IOleInPlaceFrame;
out		doc: IOleInPlaceUIWindow;
out		rcPosRect,rcClipRect: TRect;
out		frameInfo: TOleInPlaceFrameInfo
):HResult;
begin
  frame := Self;
...
function TOleCtlIntf.InsertMenus(hmenuShared: HMenu; var menuWidths: TOleMenuGroupWidths): HResult;
{var
  Menu: TMainMenu;}
begin
  {Menu := GetMainMenu;
...
function TOleCtlIntf.Invoke(dispId: Integer; const IID: TGUID; LocaleID: Integer; Flags: Word; var Params; VarResult, ExcepInfo, ArgErr: Pointer): HResult;
{var
  F: PGraphicTool;}
begin
...
function TOleCtlIntf.OleControlSite_TranslateAccelerator({$IFDEF FPC}
var	pMsg:tagMSG;
		grfModifiers:LongWord
):{$ELSE}
		msg: PMsg;
		grfModifiers:Integer
):{$ENDIF}HResult;
begin
	OleControlSite_TranslateAccelerator:=E_NOTIMPL;
end;
...
function TOleCtlIntf.OleInPlaceFrame_TranslateAccelerator(var msg: Windows.TMsg; wID: Word): HResult;
begin
  Result := S_FALSE;
end;
...
begin
  fOleCtl.FOleObject.QueryInterface( IOleInPlaceObject, fOleCtl.FOleInPlaceObject);
  fOleCtl.FOleObject.QueryInterface( IOleInPlaceActiveObject, fOleCtl.FOleInPlaceActiveObject);
  Result := S_OK;
end;
...
function TOleCtlIntf.OnPosRectChange({$IFDEF FPC}
		lprcPosRect:LPRect
):{$ELSE}
const	rcPosRect:TRect
):{$ENDIF}HResult;
var rt:TRect;
begin
	rt:=MakeRect(0, 0, 32767, 32767);
	fOleCtl.FOleInPlaceObject.SetObjectRects({$IFDEF FPC}lprcPosRect{$ELSE}rcPosRect{$ENDIF}, {$IFDEF FPC}@{$ENDIF} rt);
	OnPosRectChange:=S_OK;
end;
...
function TOleCtlIntf.PostMessageFilter({$IFDEF FPC}
		hwnd:wireHWND;
		msg:UInt;
		wp:UINT_PTR;
		lp:LONG_PTR;
...
begin
	PostMessageFilter:=S_OK;
end;

function TOleCtlIntf.PreMessageFilter({$IFDEF FPC}
		hwnd:wireHWND;
		msg:UInt;
		wp:UINT_PTR;
		lp:LONG_PTR;
...
begin
	PreMessageFilter:=S_OK;
end;

function TOleCtlIntf.QueryInterface({$IFDEF FPC}constref{$ELSE}const{$ENDIF}IID: TGUID; out Obj):HResult;
begin
	if GetInterface(IID ,Obj)
	then QueryInterface:=S_OK
	else QueryInterface:=E_NOINTERFACE
end;
...
function TOleCtlIntf.RequestBorderSpace(const borderwidths: TRect): HResult;
begin
  Result := INPLACE_E_NOTOOLSPACE;
end;
...
function TOleCtlIntf.Scroll({$IFDEF FPC}
		scrollExtant:TSIZE
):{$ELSE}
		scrollExtent: TPoint
):{$ENDIF}HResult;
...
function TOleCtlIntf.SetActiveObject(const activeObject: IOleInPlaceActiveObject; pszObjName: POleStr): HResult;
begin
  Result := S_OK;
end;

function TOleCtlIntf.SetBorderSpace({$IFDEF FPC}
const	borderwidths:TRect
):{$ELSE}
		pborderwidths:PRect
):{$ENDIF}HResult;
begin
	SetBorderSpace:=E_NOTIMPL;
end;

function TOleCtlIntf.SetMenu(hmenuShared, holemenu: HMenu; hwndActiveObject: HWnd): HResult;
var
  Menu: HMenu;
begin
  ...
  begin
    ...
    Result := OleSetMenuDescriptor( holemenu, fOleCtl.ParentForm.GetWindowHandle, hwndActiveObject, nil, nil);
  end;
end;
...
function TOleCtlIntf.TransformCoords({$IFDEF FPC}
var		pPtlHimetric:_POINTL;
var		pPtfContainer:tagPOINTF;
		dwFlags:LongWord
):HRESULT;
...
begin
	...
	PixelsPerInch:=GetDeviceCaps(DC, LOGPIXELSY);
	ReleaseDC(0, DC);
	if dwFlags and XFORMCOORDS_HIMETRICTOCONTAINER <> 0
	then begin
		pPtfContainer.X:=MulDiv(pPtlHimetric.X, PixelsPerInch, 2540);
		pPtfContainer.Y:=MulDiv(pPtlHimetric.Y, PixelsPerInch, 2540);
	end
	else begin
		pPtlHimetric.X:=Integer(Round(pPtfContainer.X*2540/PixelsPerInch));
		pPtlHimetric.Y:=Integer(Round(pPtfContainer.Y*2540/PixelsPerInch));
	end;
	TransformCoords:=S_OK;
end;
{$ELSE !FPC}
var		ptlHimetric: TPoint;
var		ptfContainer: TPointF;
