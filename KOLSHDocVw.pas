unit KOLSHDocVW;{$IFDEF FPC}{$mode objfpc}{$H+}{$ENDIF}
...
uses{$IFDEF FPC}ComObj{$ELSE}KOLComObj{$ENDIF};

procedure TWebBrowser_V1.InitControlData;
const CEventDispIDs:array[0..16]of DWORD=
(	$00000064
,	$00000065
,	$00000066
,	$0000006C
...
const CControlData:{$J+}TControlData2{$J-}=
...
begin
	...
	TControlData2(CControlData).FirstEventOfs{$IFDEF FPC}
:=	ptrUInt(addr(FOnBeforeNavigate)) - ptrUInt(addr(self)){$ELSE}
:=	Cardinal(@@FOnBeforeNavigate) - Cardinal(@Self){$ENDIF}
end;
...
begin
  CreateControl;
  GetControlInterface := FIntf;
end;

function  TWebBrowser_V1.Get_Application: IDispatch;
begin
  Get_Application := DefaultInterface.Get_Application;
end;

function  TWebBrowser_V1.Get_Parent: IDispatch;
begin
  Get_Parent := DefaultInterface.Get_Parent;
end;

function  TWebBrowser_V1.Get_Container: IDispatch;
begin
  Get_Container := DefaultInterface.Get_Container;
end;

function  TWebBrowser_V1.Get_Document: IDispatch;
begin
  Get_Document := DefaultInterface.Get_Document;
end;
...
const CEventDispIDs:array[0..17]of DWORD=
(	$00000066
,	$0000006C
,	$00000069
,	$0000006A
...
begin
	ControlData:=@ CControlData;
	TControlData2(CControlData).FirstEventOfs{$IFDEF FPC}
:=	ptrUInt(addr(FOnStatusTextChange)) - ptrUInt(addr(self)){$ELSE}
:=	Cardinal(@@FOnStatusTextChange) - Cardinal(@ self){$ENDIF}
end;
...
begin
  GetProperty := DefaultInterface.GetProperty(Property_);
end;
...
begin
  QueryStatusWB := DefaultInterface.QueryStatusWB(cmdID);
end;
